vec3 acc = glowtone_sampleSlot(glowtone_Atlas, end_sky, glowtone_ScreenProj.xy / glowtone_ScreenProj.w * 0.15).rgb * 0.4;
for (int i = 0; i < PORTAL_LAYERS; i++) {
	...
	acc += glowtone_sampleSlot(glowtone_Atlas, end_portal, uv).rgb * (0.08 + 0.02 * float(i));
}
